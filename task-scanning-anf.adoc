...
Before you can scan Azure NetApp Files volumes, https://docs.netapp.com/us-en/cloud-manager-azure-netapp-files/task-quick-start.html[BlueXP must be set up to discover the configuration^].
...
https://docs.netapp.com/us-en/cloud-manager-azure-netapp-files/task-quick-start.html[See how to discover the Azure NetApp Files system in BlueXP^].
...
Cloud Data Sense starts scanning the volumes you selected in the working environment. Results will be available in the Compliance dashboard as soon as Data Sense finishes the initial scans. The time that it takes depends on the amount of data--it could be a few minutes or hours.

NOTE: By default, if Data Sense doesn't have write attributes permissions in CIFS, or write permissions in NFS, the system won't scan the files in your volumes because Data Sense can't revert the "last access time" to the original timestamp. If you don't care if the last access time is reset, click *Or select scanning type for each volume*. That page has a setting you can enable so that Data Sense will scan the volumes regardless of permissions.
...
+
NOTE: For Azure NetApp Files, Cloud Data Sense can only scan volumes that are in the same region as BlueXP.
...
The switch at the bottom of the page for *Scan when missing "write attributes" permissions* is disabled by default. This means that if Data Sense doesn't have write attributes permissions in CIFS, or write permissions in NFS, the system won't scan the files because Data Sense can't revert the "last access time" to the original timestamp. If you don't care if the last access time is reset, turn the switch ON and all files are scanned regardless of the permissions.
