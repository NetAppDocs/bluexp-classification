...
The switch at the top of the page for *Scan when missing "write attributes" permissions* is disabled by default. This means that if Data Sense doesn't have write attributes permissions in CIFS, or write permissions in NFS, that the system won't scan the files because Data Sense can't revert the "last access time" to the original timestamp. If you don't care if the last access time is reset, turn the switch ON and all files are scanned regardless of the permissions. link:reference-collected-metadata.html#last-access-time-timestamp[Learn more^].
