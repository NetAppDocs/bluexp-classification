...
BlueXP classification is now available as a core capability within BlueXP at no additional charge for up to 500 TiB of scanned data. No Classification license or subscription is required. As we focus BlueXP classification functionality on NetApp storage systems with this new version, some legacy functionality will only be available to customers who paid for a license previously.
