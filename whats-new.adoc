...
In earlier versions of Cloud Data Sense you could have email alerts sent to the BlueXP users in your account when certain critical Policies return results. This feature enables you to get notifications to protect your data. Now you can also send email alerts to any other email address - up to 10 email addresses - that are not in your BlueXP account. 
...
Data Sense has had the ability to add custom "personal data" that Data Sense will identify in future scans for a while. However, you needed to log into the Linux host and use a command line to add the custom patterns. In this release, the ability to add personal patterns using a regex is in the Data Sense UI, making it very easy to add and edit these custom patterns.
...
In the past you could move a maximum of 100,000 source files that Data Sense is scanning to any NFS share. Now you can move up to 15 million files at a time. https://docs.netapp.com/us-en/cloud-manager-data-sense/task-managing-highlights.html#moving-source-files-to-an-nfs-share[Learn more about moving source files using Data Sense].
