...
// tag::whats-new[]
== 7 September (version 1.??)

=== Support for on-premises installations using RHEL/CentOS version 9

Docker was not supported with these Linux version...  Now using Podman version 2 or greater as the container infrastructure instead of Docker

Currently not supported in dark site installations or in distributed scanning (using a master scanner and remote node scanners)

=== Apply tags on up to 100,000 items from the Investigation Results page

Used to be one page (20 items) at a time. Now you can apply tags on up to 100,000 items at a time.

=== Identify duplicated files now has a minimum file size fo 1 MB

BlueXP classification used to identify duplicated files only for files that were 50 MB or larger. Now duplicated files starting with 1 MB are identified.
