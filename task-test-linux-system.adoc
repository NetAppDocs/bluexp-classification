...
. Enable outbound internet access from Data Sense host machine.
...
. Download the Data Sense Prerequisites script from the https://mysupport.netapp.com/site/products/all/details/cloud-data-sense/downloads-tab/[NetApp Support Site^]. The file you should select is named *test_datasense_prerequisites-<version>.tar.gz*.
...
+
[source,cli]
tar -xzf test_datasense_prerequisites-V1.0.tar.gz

. Run the script using the following command.
+
[source,cli]
test_datasense_prerequisites <--darksite>
+
