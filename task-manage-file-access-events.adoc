...
BlueXP classification can log file access events for files that reside on volumes in the following data sources:
...
TIP: You can configure file access auditing on a maximum of 5 volumes on each supported data source at this time.
...
* Client IP - IP address of the source data source
...
* User ID - name, or ID, of the person who performed the action
* File Owner ID - ID of the file owner
...
For example, the following line from the log file shows that a "Create" action has occurred in the volume "fpolicy_cifs" - a new file "f14" has been created in the volume.
 ...
 {"Timestamp": "2023-04-24 13:57", "Client_IP": "172.31.140.35", "Volume_Name": "fpolicy_cifs", "Volume_UUID": "838c1727-dd2d-11ed-a3ec-590ce4991", "File_Type": "FILE", "File_Size": 100, "Path": \\FPOLICY_CVO\fpolicy_cifs_share\dbs\f14, "Volume_Type": "SMB", "User_ID": "S-1-5-21-459977447-2546672318-3630509715-500", "File_Owner_ID": "S-1-5-32-544", "Event_Type": "CREATE", "Action_Details": {details}}
...
Each file is configured by default to contain a maximum of 50,000 events. <<Configure File Access Audit Log settings,You can customize this value in the File Access Audit Log Configuration page.>> After this maximum has been reached, the older log entries are overwritten.

The total size of all the log files in the directory is set by default to a maximum of 50 GB. <<Configure File Access Audit Log settings,You can customize this value in the File Access Audit Log Configuration page.>> When that limit is reached, the oldest log files are overwritten as new log files are added. Additionally, any log files that are older than 14 days will be overwritten as that is the maximum retention time.
