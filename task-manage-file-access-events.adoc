...
summary: Data Sense logs management activities that have been performed on files from all the working environments and data sources that Data Sense is scanning. Data Sense also logs the activities when deploying the Data Sense instance.
---
...
[.lead]
You can configure Data Sense to add events to a log file whenever a file has been accessed or changed.
...
history on files from all the working environments that Data Sense is scanning. 
The ONTAP framework creates and maintains the FPolicy configuration, monitors file events, and sends notifications to external FPolicy servers.
