...
Note that you can also link:task-quick-start-connector-on-prem.html[deploy Data Sense in an on-premises site that has internet access].
...
There is no support currently for scanning Cloud Volumes ONTAP, Azure NetApp Files, FSx for ONTAP, AWS S3, or Google Drive, OneDrive, or SharePoint Online accounts when Data Sense is deployed in a dark site.
...
| Disk
a| *Large* | 16 CPUs | 64 GB RAM | 500 GiB SSD on /, or
