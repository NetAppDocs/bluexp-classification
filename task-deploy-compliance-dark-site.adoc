...
[role="quick-margin-para"]
The first 1 TB of data that Cloud Data Sense scans in BlueXP is free. A BYOL license from NetApp is required to continue scanning data after that point.
...
* Ensure that BlueXP has permissions to deploy resources and create security groups for the Cloud Data Sense instance.
* Ensure that the BlueXP Connector can access the Data Sense instance. The security group for the Connector must allow inbound and outbound traffic over port 443 to and from the Data Sense instance.
+
...
* Depending on the types of data sources you plan to scan, additional ports must be open to the Data Sense instance. See the details for each data source you plan to scan.
...
+
This extracts required software and the actual installation file *DATASENSE-INSTALLER-V1.16.1.tar.gz*.
...
.. Copy the Data Sense installer file (*DATASENSE-INSTALLER-<version>.tar.gz*) to the host machine.
...
* Data Sense software can be upgraded one major version at a time. For example, if you have version 1.15.x installed, you can upgrade only to 1.16.x. If you are a few major versions behind, you'll need to upgrade the software multiple times.
...
+
This extracts the installation file *DATASENSE-INSTALLER-V1.16.1.tar.gz*.
