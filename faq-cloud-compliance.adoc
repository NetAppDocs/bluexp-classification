...
NetApp does not have access to sensitive data and does not display it. Sensitive data is masked. For example, credit card information shows only the last 4 digits. 
...
BlueXP classification captures metadata such as: file name, permissions, creation time, last access, and last modification. 
...
BlueXP classification accesses ElasticSearch through API calls, which require authentication and are encrypted using AES-128. Accessing ElasticSearch directly requires root access. 
