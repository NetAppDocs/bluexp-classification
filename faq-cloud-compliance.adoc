...
Cloud Data Sense deploys a single server or cluster wherever the user chooses—in the cloud or on premises. The servers connect via standard protocols to the data sources and index the findings in an Elasticsearch cluster, which is also deployed on the same servers. This allows support for multicloud, cross-cloud, private cloud, and on-premises environments.
...
Cloud Manager has implementations that are available as first-party services in all of the major hyperscalers (AWS FSx for NetApp ONTAP in AWS; Azure NetApp Files in Azure; Cloud Volumes Service in Google Cloud). All of these implementations leverage Cloud Manager services such as Cloud Data Sense and can be managed with features enabled on a point-and-click basis through our single-pane-of-glass UI, Cloud Manager. Services such as Cloud Data Sense can be seamlessly enabled by any authorized Cloud Manager user who is using one of the first-party offerings from a major hyperscaler. In addition, the services are available in all marketplaces, and they are authorized co-sell offerings with the hyperscalers, which means that the hyperscaler sales teams can receive compensation when these services are enabled in their accounts. All of these factors spread Cloud Manager services throughout the ecosystem and expand our reach.

These first-party offerings give us priority access to the millions of users across the hyperscaler marketplaces who need world-class data services like NetApp Cloud Data Sense, Cloud Backup, Cloud Tiering, Cloud Sync, Global File Cache, and more. It's easier than ever for users to transact and deploy services that complement and complete their cloud portfolio from all marketplaces.
...
=== Can I search on my data with Cloud Data Sense?

The Cloud Data Sense service offers extensive search capabilities that make it easy to search for a specific file or piece of data across all connected sources. Cloud Data Sense empowers users to search deeper than just what the metadata reflects. It is a language-agnostic service that can also read the files and analyze a multitude of sensitive data types, such as names and IDs. For example, users can search across both structured and unstructured data stores to find data that may have leaked from databases to user files, in violation of corporate policy. Searches can be saved for later, and policies can be created to search and take action on the results at a set frequency.

Once the files of interest are found, characteristics can be listed, including tags, working environment account, bucket, file path, category (from classification), file size, last modified, permission status, duplicates, sensitivity level, personal data, sensitive data types within the file, owner, file type, file size, created time, file hash, whether the data was assigned to someone seeking their attention, and more. Filters can be applied to screen out characteristics that are not pertinent. Cloud Data Sense also has RBAC controls to allow files to be moved or deleted, if the right permissions are present. If the right permissions are not present, the tasks can be assigned to someone in the organization who does have the right permissions.

=== What kind of analytics does Cloud Data Sense provide?

Data sources can be represented visually, and relationships defined and depicted graphically. For example, admins can see all stale, duplicate, and non-business-related data across data sources throughout the enterprise (on-premises systems, databases, cloud file shares, S3 stores, OneDrive, etc.). They can then move, delete, and manage data to optimize storage costs and reduce risk. Users can reduce risk by seeing what sensitive data might be exposed, and they can create jobs to manage permissions for strong data protection. The Cloud Data Sense service also classifies all the different types of data, so admins can investigate data by type and see what actions have been taken on the data, and when.
...
Cloud Manager allows the user to scan and report on systems virtually anywhere, including on-premises, cloud, and hybrid environments. Cloud Data Sense is normally deployed using a SaaS model, in which the service is enabled via the Cloud Manager interface and requires no hardware or software installation. Even in this click-and-run deployment mode, the data management, including synchronization, tiering, backup and restore, security, governance, and compliance, can be done regardless of whether the data stores are on premises or in the public cloud.

For secure installations, Cloud Manager and Cloud Data Sense can be deployed in a “dark site” model, which is installed as a package on premises and requires no external network connections.

Regardless of deployment method (on premises or SaaS-delivered cloud model), the user has full control over configuration of installation. No information is sent to NetApp; full processing is encapsulated in the server.
