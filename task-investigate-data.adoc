...
| File Path | Enter up to 20 partial or full paths that you want to include or exclude from the query. If you enter both include paths and exclude paths, Data Sense finds all files in the included paths first, then it removes files from excluded paths, and then it displays the results. Note that including "*" in this filter has no effect; you'll need to search for specific files and subfolders by using full paths.
