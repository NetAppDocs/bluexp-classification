...
.. Run the following script to register this information with all Scanner nodes: update_we_scanner_group_from_config_file.sh
...
* You must have the IP address or host name of the Connector system, your NetApp Account ID, Connector Client ID, and user access token. See _Prerequisite steps_ below to get this information.
* You'll need to know the _working environment ID_ for each data source in your account. Contact NetApp at ng-contact-data-sense@netapp.com with your request.
* The following ports and protocols must be enabled on all hosts:
+
[cols="15,20,55",options="header"]
|===
...
+
When the installation finishes on all scanner nodes and they have been joined to the manager node, the "add_scanner_node.sh" finishes as well. The installation can take 10 to 20 minutes.

. If you added any scanner nodes into a scanner group, return to the Manager node and perform the following 2 tasks:
.. Open the file "/opt/netapp/Datasense/working_environment_to_scanner_group_config.yml" and enter a mapping for which scanner groups will scan specific working environments. For example, the following entries add 2 working environments into 2 scanner groups:
+
 scanner group:
  europe:
...
.What's Next
From the Configuration page you can select the data sources that you want to scan. If you created scanner groups, each data source is scanned by the Scanner nodes in the respective group.
