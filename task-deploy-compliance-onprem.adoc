...
summary: Complete a few steps to deploy Data Sense on a Linux host in your network, or on a Linux host in the cloud, that has internet access.
---
...
[.lead]
Complete a few steps to deploy Data Sense on a Linux host in your network, or on a Linux host in the cloud, that has internet access.
...
If you don't already have a Connector, https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-installing-linux.html[deploy the Connector on-premises^] on a Linux host in your network, or on a Linux host in the cloud.
...
// Add this next release. And add to dark site topic too.  Change /var to /var/lib/docker for early May release.
