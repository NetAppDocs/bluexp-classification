...
For versions 1.30 and earlier, if you need to install BlueXP classification on multiple hosts, refer to link:task-deploy-multi-host-install-dark-site.html[Install BlueXP classification on multiple hosts with no internet access].
...
[role="quick-margin-para"]
The first 500 TB of data that BlueXP classification scans in BlueXP is free for 30 days. A subscription to your cloud provider Marketplace, or a BYOL license from NetApp, is required to continue scanning data after that point.
...
On-prem ONTAP systems, non-NetApp file shares and database accounts can be scanned using any of these cloud Connectors.
...
You can also link:task-licensing-datasense.html[set up licensing for BlueXP classification] at this time. You will not be charged even after your 30-day free trial ends.
