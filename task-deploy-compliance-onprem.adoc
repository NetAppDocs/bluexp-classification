...
.. Open the file "/opt/netapp/Datasense/working_environment_to_scanner_group_config.yml" and enter the mapping for which scanner groups will scan specific working environments. You'll need to get the _working environment ID_ for each data source from NetApp (email ng-contact-data-sense@netapp.com). For example, the following entries add 2 working environments into 2 scanner groups:
+
 scanner group:
  europe:
...
+
Any working environment that is not added to the list is scanned by the "default" group - if a scanner node exists for the "default" group.
//+
