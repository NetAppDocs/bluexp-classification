...
= Deploy Cloud Data Sense on a host that has internet access
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
. The _Deploy Data Sense On Premises_ dialog is displayed. Copy the provided command and paste it in a text file so you can use it later, and click *Close*. For example:
+
