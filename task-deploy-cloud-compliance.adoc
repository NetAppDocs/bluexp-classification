...
Note that when deploying Data Sense from a BlueXP Connector in an AWS environment, you can select the default instance size or you can select from two smaller instance types. link:concept-cloud-compliance.html#using-a-smaller-instance-type[See the available instance types and limitations]. In regions where the default instance type isn't available, Data Sense runs on an link:reference-instance-types.html[alternate instance type].
