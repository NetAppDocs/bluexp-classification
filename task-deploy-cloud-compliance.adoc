...
Note that when deploying Data Sense from a BlueXP Connector in an AWS environment, you can select the default instance size or two smaller instance types. link:concept-cloud-compliance.html#using-a-smaller-instance-type[See the available instance types and limitations].
