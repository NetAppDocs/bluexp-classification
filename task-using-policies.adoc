...
To display the results for a policy in the Investigation page, select the image:screenshot_gallery_options.gif[More button] button for a specific policy, and then select *Investigate Results*.
...
. You can alter the name and the description. To limit the changes to these fields, select *Save Policy*.
+
If you want to change the filters for the saved query, select *Edit Query*.
+
...
. In the Investigation page that defines that query, edit the query by adding, removing, or customizing the filters, and select *Save Changes* .
+
