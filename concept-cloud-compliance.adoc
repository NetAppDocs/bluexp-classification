...
BlueXP classification uses two types of scans: 

* A general "mapping-only" scan on selected data sources. Mapping-only scans provide only a high-level overview of your data. Mapping can be done on your data sources very quickly because it does not access files to see the data inside.
