...
[.lead]
The first 1 TB of data that Cloud Data Sense scans in a BlueXP workspace is free for 30 days. A BYOL license from NetApp, or a subscription from your cloud provider's marketplace, is required to continue scanning data after that point.

A few notes before you read any further:

* If you've already subscribed to the BlueXP pay-as-you-go (PAYGO) subscription in your cloud provider's marketplace, then you're automatically subscribed to Cloud Data Sense as well. You won't need to subscribe again.
...
You can subscribe at any time and you will not be charged until either the 30 day trial ends, or the amount of data exceeds 1 TB. You can always see the total amount of data that is being scanned from the Data Sense Dashboard. And the _Subscribe Now_ button makes it easy to subscribe when you are ready.
...
NOTE: If the account you are using has both a BYOL license and a PAYGO subscription, Data Sense _will not_ shift over to the PAYGO subscription when the BYOL license expires. You must renew the BYOL license.
