...
| File Path | Enter up to 20 partial or full paths that you want to include or exclude from the query. If you enter both paths to include and exclude, Data Sense finds all files in the included paths first, then it removes files from excluded paths, and then it displays the results.
...
| Last Accessed | Select a time range when the file was last accessed. You can also specify a custom time range to further refine the search results. For the types of files that Data Sense scans, this is the last time Data Sense scanned the file.
