...
* You can send email alerts to BlueXP users when certain critical Policies return results.
...
.. Optionally, check the box if you want notification emails sent to BlueXP users, and choose the interval at which the email is sent. Learn more about link:task-org-private-data.html#sending-email-alerts-when-non-compliant-data-is-found[sending email alerts based on policy results].
...
Cloud Data Sense can send email alerts to BlueXP users when certain critical Policies return results so you can get notifications to protect your data. You can choose to send the email notifications on a daily, weekly, or monthly basis.
...
. In the Edit Policy page, check the box if you want notification emails sent to BlueXP users, and choose the interval at which the email is sent (for example, every *Week*).
+
